...
export interface LevelUpConfig { title?: string; description?: string; imageUrl?: string; color?: number; }
export interface LevelingConfig { xpMin: number; xpMax: number; cooldownSeconds: number; multiplier: number; bonusXp: number; levelUp?: LevelUpConfig; ignoreChannels: string[]; ignoreRoles: string[]; roleRewards: Record<string, string>; }
export interface TicketPanelOption { id: string; name: string; description: string; categoryId?: string; supportRoleId?: string; questions: string[]; }
export interface TicketPanelConfig { id: string; name: string; title: string; description: string; color?: number; categoryId?: string; supportRoleId?: string; questions: string[]; options?: TicketPanelOption[]; }
export interface Config { logChannel?: string; transcriptLogChannel?: string; inviteLogChannel?: string; muteRole?: string; jailRole?: string; chatBanRole?: string; ticketCategory?: string; ticketSupportRole?: string; ticketPanels?: Record<string, TicketPanelConfig>; starboardChannel?: string; starboardThreshold: number; levelChannel?: string; leveling?: LevelingConfig; snipeEnabled: boolean; prefix?: string; levelRoles?: Record<string, string>; inviteRoles?: Record<string, string>; levelUpMessage?: { title?: string; description?: string; imageUrl?: string; }; automod?: AutoModConfig; moderationTemplates?: ModerationTemplate[]; logging?: LoggingConfig; giveawayDaily?: GiveawayDailyConfig; joinRole?: JoinRoleConfig; socialNotifications?: SocialNotificationConfig; reactionRoles?: ReactionRoleConfig; roleConnections?: RoleConnectionConfig; youtubeNotifications?: YouTubeNotification[]; aiPersonality?: 'funny' | 'roaster' | 'chill' | 'nerd' | 'savage'; aiChannelId?: string; bloxValueChannelId?: string; bloxScannerChannelId?: string; bloxScannerScreenshots?: Record<string, BloxScannerScreenshot>; bloxValueEmojis?: Record<string, string>; }
...
export interface SavedEmbed { name: string; sourceKey?: string; overrideFields?: Array<'title'|'description'|'color'|'thumbnailUrl'|'imageUrl'|'footerText'|'footerIconUrl'|'authorName'|'authorIconUrl'|'timestamp'|'fields'>; base?: Omit<SavedEmbed,'base'>; placeholder?: boolean; title?: string; description?: string; color?: number; thumbnailUrl?: string; imageUrl?: string; footerText?: string; footerIconUrl?: string; authorName?: string; authorIconUrl?: string; timestamp?: boolean; fields?: EmbedField[]; }
...
export interface Giveaway { id: string; guildId: string; channelId: string; messageId: string; name: string; prize: string; endsAt: number; hostId: string; donorId?: string; winnerCount?: number; winnerIds?: string[]; type?: string; pingRoleId?: string; customMessage?: string; hideEntryCount?: boolean; durationStr?: string; entries: string[]; requiredRoleId?: string; bypassRoleId?: string; blacklistRoleId?: string; imageUrl?: string; ended: boolean; winnerId?: string; }
...
export interface RecoveryChannel { id: string; name: string; type: number; position: number; parentId?: string; topic?: string; nsfw?: boolean; rateLimitPerUser?: number; userLimit?: number; rtcRegion?: string; videoQualityMode?: number; defaultAutoArchiveDuration?: number; defaultThreadRateLimitPerUser?: number; defaultForumLayout?: number; defaultSortOrder?: number; permissionOverwrites: RecoveryOverwrite[]; }
...
export interface RecoveryConfig { enabled: boolean; intervalMinutes: number; lastAutoBackupAt?: number; }
export interface ActivityDay { count: number; words: Record<string, number>; }
export interface ActivityUser { totalMessages: number; activeDays: number; firstSeen: number; lastSeen: number; daily: Record<string, ActivityDay>; monthly: Record<string, number>; }
export interface ActivityData { startedAt: number; totalMessages: number; users: Record<string, ActivityUser>; }
export interface GuildData { config: Config; antiNuke: AntiNukeConfig; extraOwners: string[]; afk: Record<string, AfkEntry>; levels: Record<string, LevelEntry>; activity: ActivityData; sparks: Record<string, number>; invites: Record<string, number>; inviteSources: Record<string, string>; warnings: Record<string, Warning[]>; reminders: Reminder[]; starboard: Record<string, StarboardEntry>; lastDeleted: Record<string, SnipedMessage[]>; lastEdited: Record<string, SnipedMessage[]>; tempRoles: TempRole[]; tickets: Record<string, Ticket>; autoResponders: AutoResponder[]; giveaways: Giveaway[]; savedEmbeds: Record<string, SavedEmbed>; welcome?: WelcomeConfig; shop: ShopConfig; recovery: RecoveryConfig; recoveryBackups: RecoveryBackup[]; }
