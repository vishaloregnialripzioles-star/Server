...
import type { TicketPanelConfig } from './types.js';
...
export function getTicketPanel(guild:Guild,panelId?:string):TicketPanelConfig|undefined {
  ...
  const first=Object.values(panels)[0];
  return first;
}

export async function createTicketForUser(guild:Guild,user:User,client:Client,reason:string,panelId?:string,answers?:Record<string,string>):Promise<TicketResult>{
  const data=loadGuild(guild.id);const panel=getTicketPanel(guild,panelId);
  ...
  const supportRoleId=panel?.supportRoleId??data.config.ticketSupportRole;const categoryId=panel?.categoryId??data.config.ticketCategory;
  ...
  try{
    ...
    updateGuild(guild.id,d=>{d.tickets[ticketId]={id:ticketId,channelId:ticketChannel.id,creatorId:user.id,createdAt:Date.now(),closed:false,panelId:panel?.id};});
    const embed=new EmbedBuilder().setColor(panel?.color??0x5865F2).setTitle(panel?.title?`🎫 ${panel.title}`:'🎫 Support Ticket Opened').setDescription(`Hello <@${user.id}>! Your ticket is open.\n\n**Reason:**\n${reason}`).addFields({name:'Ticket ID',value:`\`${ticketId}\``,inline:true},{name:'Handler',value:'Unclaimed',inline:true}).setTimestamp();
...
export async function setTicketClaim(guild:Guild,ticketId:string,staffId:string|null):Promise<{ok:boolean;message:string}>{
  ...
  const panel=(data.config.ticketPanels??{})[ticket.panelId??''];const supportRoleId=panel?.supportRoleId??data.config.ticketSupportRole;const channel=await guild.channels.fetch(ticket.channelId).catch(()=>null) as TextChannel|null;if(!channel)return{ok:false,message:'Ticket channel no longer exists.'};
