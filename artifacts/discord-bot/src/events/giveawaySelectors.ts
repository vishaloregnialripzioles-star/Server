...
function channelPicker(userId: string): ActionRowBuilder<ChannelSelectMenuBuilder> {
  return new ActionRowBuilder<ChannelSelectMenuBuilder>().addComponents(
    new ChannelSelectMenuBuilder()
      .setCustomId(`gws_pick_channel:${userId}`)
      .setPlaceholder('Select the giveaway channel…')
...
function multiplierBonusModal(userId: string, roleId: string): ModalBuilder {
  const input = new TextInputBuilder()
    ...
    .setLabel('Entry multiplier (1–100×)')
    .setStyle(TextInputStyle.Short)
    .setPlaceholder('Example: 2')
    .setRequired(true)
    .setMaxLength(3);
  return new ModalBuilder()
    .setCustomId(`gws_multiplier_bonus:${userId}:${roleId}`)
...
async function finishGiveaway(interaction: any, pending: NonNullable<ReturnType<typeof pendingGiveaways.get>>): Promise<void> {
  if (!pending.channelId) {
    await renderConfig(interaction, pending.hostId, pending, 'Please select a giveaway Channel before finishing.');
    return;
  }
...
export async function handleGiveawaySelectors(interaction: Interaction): Promise<void> {
  ...
  if (!pending) {
    if ('reply' in interaction) await (interaction as any).reply({ content: '❌ Setup session expired. Run `/giveaway create` again.', flags: 64 });
    return;
  }
  ...
  if (interaction.isButton()) {
    ...
    if (customId === `gws_multipliers:${userId}`) {
      await interaction.update({ embeds: [selectorEmbed('✨ Entry Multipliers', 'Give members with selected roles extra weighted chances. Add a role, then choose its multiplier.')], components: multiplierRows(userId, pending) });
      return;
    }
    if (customId === `gws_channel:${userId}`) {
      await interaction.update({ embeds: [selectorEmbed('📣 Giveaway Channel', 'Select where the giveaway will be posted.')], components: [channelPicker(userId), backRow(userId)] });
      return;
    }
    if (customId === `gws_pingrole:${userId}`) {
      await interaction.update({ embeds: [selectorEmbed('🔔 Ping Role', 'Select the role to notify when the giveaway starts, or choose @everyone.')], components: [rolePicker(userId, 'pingrole', 'Select a role to ping…'), new ActionRowBuilder<ButtonBuilder>().addComponents(new ButtonBuilder().setCustomId(`gws_ping_everyone:${userId}`).setLabel('@everyone').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId(`gws_clear_ping:${userId}`).setLabel('Clear').setStyle(ButtonStyle.Danger)), backRow(userId)] });
      return;
    }
    if (customId === `gws_donor:${userId}`) {
      await interaction.update({ embeds: [selectorEmbed('🤝 Giveaway Donor', 'Select the donor account directly from the server.')], components: [donorPicker(userId), new ActionRowBuilder<ButtonBuilder>().addComponents(new ButtonBuilder().setCustomId(`gws_clear_donor:${userId}`).setLabel('Clear Donor').setStyle(ButtonStyle.Danger)), backRow(userId)] });
      return;
    }
    const limiterAction = ['required', 'bypass', 'blacklist'].find(k => customId === `gws_${k}:${userId}`);
    if (limiterAction) {
      const labels: Record<string, string> = { required: 'Required Role', bypass: 'Requirement Bypass Role', blacklist: 'Blacklist Role' };
      await interaction.update({ embeds: [selectorEmbed(`🛡️ ${labels[limiterAction]}`, 'Select a role from this server. The selected role will be applied immediately.')], components: [rolePicker(userId, limiterAction, `Select ${labels[limiterAction]}…`), backRow(userId)] });
      return;
    }
    const clear = ['required', 'bypass', 'blacklist'].find(k => customId === `gws_clear_${k}:${userId}`);
    if (clear) {
      if (clear === 'required') pending.requiredRoleId = undefined;
      if (clear === 'bypass') pending.bypassRoleId = undefined;
      if (clear === 'blacklist') pending.blacklistRoleId = undefined;
      await interaction.update({ embeds: [selectorEmbed('✅ Updated', 'The requirement was cleared. Returning to the requirements panel…')], components: limiterRows(userId, pending) });
      return;
    }
    if (customId === `gws_add_multiplier:${userId}`) {
      await interaction.update({ embeds: [selectorEmbed('➕ Add Multiplier Role', 'Select the role that should receive extra weighted chances.')], components: [rolePicker(userId, 'multiplier', 'Select multiplier role…'), backRow(userId)] });
      return;
    }
    if (customId === `gws_clear_multipliers:${userId}`) {
      pending.extraEntryRoles = [];
      await interaction.update({ embeds: [selectorEmbed('✅ Multipliers Cleared', 'All multiplier roles have been removed.')], components: multiplierRows(userId, pending) });
      return;
    }
    if (customId.startsWith(`gws_remove_multiplier:${userId}:`)) {
      const index = Number(customId.split(':').pop());
      if (Number.isInteger(index) && index >= 0) pending.extraEntryRoles.splice(index, 1);
      await interaction.update({ embeds: [selectorEmbed('✨ Entry Multipliers', 'Multiplier updated.')], components: multiplierRows(userId, pending) });
      return;
    }
  ...
  if (interaction.isRoleSelectMenu()) {
    ...
    if (roleId === interaction.guild.id) {
      await interaction.reply({ content: '❌ @everyone cannot be used for this setting.', flags: 64 });
      return;
    }
    if (customId === `gws_pick_required:${userId}`) pending.requiredRoleId = roleId;
  ...
  if (interaction.isChannelSelectMenu() && customId === `gws_pick_channel:${userId}`) {
  ...
  if (interaction.isUserSelectMenu() && customId === `gws_pick_donor:${userId}`) {
    pending.donorId = interaction.values[0];
    await renderConfig(interaction, userId, pending);
    return;
  }
  ...
  if (interaction.isModalSubmit() && customId.startsWith(`gws_multiplier_bonus:${userId}:`)) {
    ...
    if (!roleId || !Number.isInteger(multiplier) || multiplier < 1 || multiplier > 100) {
      await interaction.reply({ content: '❌ Multiplier must be a whole number from 1 to 100.', flags: 64 });
      return;
    }
    const existing = pending.extraEntryRoles.find((r: ExtraEntryRole) => r.roleId === roleId);
    if (existing) existing.entries = multiplier;
    else pending.extraEntryRoles.push({ roleId, entries: multiplier });
    await interaction.reply({ embeds: [selectorEmbed('✅ Multiplier Saved', `<@&${roleId}> now has a **${multiplier}×** entry multiplier.`)], components: [new ActionRowBuilder<ButtonBuilder>().addComponents(new ButtonBuilder().setCustomId(`gws_back:${userId}`).setLabel('← Back to Giveaway Setup').setStyle(ButtonStyle.Primary))], flags: 64 });
    return;
  }
}
