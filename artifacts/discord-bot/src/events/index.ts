...
client.on(Events.MessageCreate,safe('messageCreate',async(message:any)=>{if(message?.[MESSAGE_PROCESSED])return;message[MESSAGE_PROCESSED]=true;const{handleMessageCreate}=await import('./messageCreate.js');return handleMessageCreate(message);}));
