import { SlashCommandBuilder, PermissionFlagsBits, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, type TextChannel } from 'discord.js';
...
export const ticketpanel: Command = {
  ...
  async execute(interaction){
    ...
    const row=new ActionRowBuilder<ButtonBuilder>().addComponents(new ButtonBuilder().setCustomId(`ticket:open:${panel.id}`).setLabel('Open Ticket').setEmoji('🎫').setStyle(ButtonStyle.Primary));
    try{await (channel as unknown as TextChannel).send({embeds:[embed],components:[row]});await interaction.editReply(`✅ Panel **${name}** sent to <#${channel.id}>.`);}catch{await interaction.editReply('❌ I could not send the panel. Check channel permissions.');}
  },
};
