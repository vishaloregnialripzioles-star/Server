...
export const giveaway: Command = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(sub =>
      sub
        .setName('create')
        .setDescription('Open the giveaway setup panel'),
    )
    .addSubcommand(sub =>
      sub
        .setName('reroll')
  ...
  async execute(interaction) {
    ...
    // /giveaway create intentionally has NO name/prize/duration/channel options.
    // Everything is configured after sending the command using the setup buttons.
    if (sub === 'create') {
      ...
      await interaction.reply({
        embeds: [buildConfigEmbed(pending)],
