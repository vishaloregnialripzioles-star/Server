...
const EMOJIS: Record<string,string> = { Setup:'⚙️',Moderation:'🔨',Security:'🛡️',Channels:'📢',Restrictions:'🚫',Utility:'🛠️',Profiles:'🖼️',Leveling:'📈',Roles:'🎭',Tickets:'🎫',Economy:'⚡',Games:'🎮',Fun:'🎉',Giveaways:'🎁',Music:'🎵',Social:'🌐','Blox Fruits':'🍎',AI:'🤖',Other:'✨' };
...
function displayName(name:string,prefix:string):string { return `${prefix}${name}`; }
function optionEntries(data:any,prefix:string):{name:string;description:string}[] {
  const out:{name:string;description:string}[]=[];
  for(const option of data.options ?? []) {
    if(option.type===1) out.push({name:`${prefix}${data.name} ${option.name}`,description:option.description ?? 'Command option'});
...
export function getHelpCategories(commands:Command[]=commandRegistry,prefix='.'):HelpCategory[] {
  const buckets=new Map<string,HelpEntry[]>(); for(const c of CATEGORY_ORDER)buckets.set(c,[]);
  for(const command of commands) {
    const data:any=command.data.toJSON(); const name=String(data.name); const category=categoryFor(name); const premium=PREMIUM.has(name.toLowerCase());
    buckets.get(category)!.push({name:displayName(name,prefix),description:String(data.description ?? 'No description available.'),premium});
...
export function findHelpCategory(category?:string|null,commands:Command[]=commandRegistry,prefix='.'):HelpCategory|undefined { if(!category||category.toLowerCase()==='all')return undefined; return getHelpCategories(commands,prefix).find(c=>c.name.toLowerCase()===category.toLowerCase()); }
function formatCommand(entry:HelpEntry):string { return `• ${entry.name}${entry.premium?' 🟨':''} — ${entry.description}`; }
export function buildHelpEmbed(category?:string|null,prefix='.',commands:Command[]=commandRegistry):EmbedBuilder {
  const categories=getHelpCategories(commands,prefix); const selected=category?categories.find(c=>c.name.toLowerCase()===category.toLowerCase()):undefined; const count=categories.reduce((n,c)=>n+c.commands.length,0);
  ...
  else { embed.setTitle(`${selected.emoji} ${selected.name}`).setDescription(`${selected.commands.length} command entries.\n🟨 = Premium.`); for(let i=0;i<selected.commands.length;i+=10) embed.addFields({name:i?'More commands':'Commands',value:selected.commands.slice(i,i+10).map(formatCommand).join('\n').slice(0,1024)}); }
  return embed;
}
export function buildHelpMenu(category?:string|null,commands:Command[]=commandRegistry,prefix='.'):ActionRowBuilder<StringSelectMenuBuilder> {
  const categories=getHelpCategories(commands,prefix); return new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(new StringSelectMenuBuilder().setCustomId(HELP_SELECT_CUSTOM_ID).setPlaceholder('Select a category…').addOptions({label:'All commands',description:'Browse the complete command directory',value:'all',default:!category||category==='all',emoji:'📚'},...categories.slice(0,24).map(c=>({label:c.name,description:`${c.commands.length} entries`,value:c.name.toLowerCase(),default:category?.toLowerCase()===c.name.toLowerCase(),emoji:c.emoji}))));
}
