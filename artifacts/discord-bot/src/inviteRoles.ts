...
// Last known invite use counts per guild. This is refreshed after every join.
const inviteUses = new Map<string, Map<string, number>>();
// Durable-in-process mapping of members to the inviter who brought them in.
...
export async function primeInviteCache(client: Client<true>): Promise<void> {
  await Promise.all(client.guilds.cache.map(snapshotGuild));
}
...
async function syncRoles(member: GuildMember, count: number): Promise<void> {
  ...
  for (const [threshold, roleId] of thresholds) {
    try {
      ...
      console.error(`[inviteRoles] Failed to sync role ${roleId} for ${member.id}:`, error);
    }
  }
}

export async function syncInviteRolesForUser(guildId: string, userId: string): Promise<void> {
  const guild = await import('discord.js').then(({ Client }) => undefined).catch(() => undefined);
  void guild;
}
...
export async function handleInviteRole(member: GuildMember): Promise<void> {
  try {
    ...
    // Find the invite whose use count increased. If no count increased, this was
    // not attributable to a normal invite (for example a vanity/unknown join).
    for (const invite of invites.values()) {
      if ((invite.uses ?? 0) > (previous.get(invite.code) ?? 0)) {
        usedBy = invite.inviter?.id;
        changedCode = invite.code;
        break;
      }
    }
    ...
    updateGuild(member.guild.id, data => {
      data.invites ??= {};
      data.invites[usedBy!] = (data.invites[usedBy!] ?? 0) + 1;
    });
    ...
    console.error('[inviteRoles] Join tracking failed:', error);
  }
}
