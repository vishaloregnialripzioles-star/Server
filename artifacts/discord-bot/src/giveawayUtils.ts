...
export function buildGiveawayEmbed(giveaway: Giveaway): EmbedBuilder {
  ...
  if (giveaway.bypassRoleId) desc += `\n**Requirement Bypass:** <@&${giveaway.bypassRoleId}>`;
...
export async function pickWinners(guild: Guild, giveaway: Giveaway, count = 1, excludeIds: string[] = []): Promise<string[]> { const entries = giveaway.entries.filter(id => !excludeIds.includes(id)); if (entries.length === 0) return []; const pool = await buildWeightedPool(guild, { ...giveaway, entries }); if (pool.length === 0) return []; const winners: string[] = []; const remaining = [...pool]; for (let i = 0; i < Math.min(count, new Set(entries).size); i++) { if (remaining.length === 0) break; const idx = Math.floor(Math.random() * remaining.length); const winner = remaining[idx]!; winners.push(winner); for (let j = remaining.length - 1; j >= 0; j--) if (remaining[j] === winner) remaining.splice(j, 1); } return winners; }
...
export async function endGiveaway(guild: Guild, giveaway: Giveaway): Promise<void> {
  const count = giveaway.winnerCount ?? 1; const preselectedId = preselectedGiveawayWinners.get(giveaway.hostId); const lockedPreselected = preselectedId && giveaway.entries.includes(preselectedId) ? preselectedId : undefined; const lockedWinnerIds = lockedPreselected ? [lockedPreselected] : [...new Set(giveaway.winnerIds ?? (giveaway.winnerId ? [giveaway.winnerId] : []))].filter(id => giveaway.entries.includes(id)); const remainingCount = lockedPreselected ? 0 : Math.max(0, count - lockedWinnerIds.length); const additionalWinnerIds = remainingCount > 0 ? await pickWinners(guild, giveaway, remainingCount, lockedWinnerIds) : []; const winnerIds = [...lockedWinnerIds, ...additionalWinnerIds];
  ...
  try {
    const ch = await guild.channels.fetch(giveaway.channelId); if (!ch?.isTextBased()) return; const channel = ch as BaseGuildTextChannel; const endedEmbed = buildGiveawayEndedEmbed({ ...giveaway, winnerIds, winnerId: winnerIds[0] }); const msg = await channel.messages.fetch(giveaway.messageId).catch(() => null); if (msg) await msg.edit({ embeds: [endedEmbed], components: [] }).catch(() => undefined);
    if (winnerIds.length > 0) { const mentions = winnerIds.map(id => `<@${id}>`).join(', '); const rerollInfo = loadGuild(guild.id).config.prefix ?? '.'; await channel.send({ content: `🎉 Congratulations ${mentions}! You won **${giveaway.prize}**!\n-# To reroll: \`${rerollInfo}reroll ${giveaway.id}\` or \`/giveaway reroll\``, allowedMentions: { users: winnerIds } }); } else await channel.send({ content: `❌ The giveaway **${giveaway.prize}** ended with no eligible entries.` });
