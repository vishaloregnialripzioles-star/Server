...
export function buildGiveawayEmbed(giveaway: Giveaway): EmbedBuilder {
  ...
  if (giveaway.extraEntryRoles && giveaway.extraEntryRoles.length > 0) {
    desc += `\n\nMultipliers`;
    for (const er of giveaway.extraEntryRoles) desc += `\n<@&${er.roleId}>: +${er.entries} entries`;
  }
  ...
  const embed = new EmbedBuilder().setColor(0xFAA61A).setTitle(giveaway.prize).setDescription(desc).setFooter({ text: footerText });
...
export function buildGiveawayRow(giveawayId: string, entryCount = 0, hideCount = false, includeWinnerButton = true): ActionRowBuilder<ButtonBuilder> {
  ...
  if (includeWinnerButton) {
    row.addComponents(
      new ButtonBuilder().setCustomId(`giveaway_select_winner:${giveawayId}`).setLabel('🎯 Select Winner').setStyle(ButtonStyle.Primary),
    );
  }
  return row;
}
...
export function buildGiveawayEndedEmbed(giveaway: Giveaway): EmbedBuilder {
  ...
  const winnerMentions = winnerIds.length > 0 ? winnerIds.map(id => `<@${id}>`).join(', ') : '*(No eligible entries)*';
  const embed = new EmbedBuilder()
    .setColor(0xED4245)
    .setTitle(`🎉 ${giveaway.prize} — ENDED`)
    .setDescription(`**Winner${winnerIds.length !== 1 ? 's' : ''}:** ${winnerMentions}\n\n**Total Entries:** ${giveaway.entries.length}\nHosted by: <@${giveaway.hostId}>${giveaway.donorId ? `\nDonor: <@${giveaway.donorId}>` : ''}`)
...
async function buildWeightedPool(guild: Guild, giveaway: Giveaway): Promise<string[]> {
  ...
  for (const userId of giveaway.entries) {
    ...
    if (giveaway.extraEntryRoles && giveaway.extraEntryRoles.length > 0) {
      ...
      if (member) for (const extra of giveaway.extraEntryRoles) if (member.roles.cache.has(extra.roleId)) weight = Math.max(weight, extra.entries);
    }
...
export async function pickWinners(guild: Guild, giveaway: Giveaway, count = 1, excludeIds: string[] = []): Promise<string[]> {
  ...
  for (let i = 0; i < Math.min(count, new Set(entries).size); i++) {
    ...
    winners.push(winner);
    for (let j = remaining.length - 1; j >= 0; j--) if (remaining[j] === winner) remaining.splice(j, 1);
  }
  return winners;
}
...
export async function endGiveaway(guild: Guild, giveaway: Giveaway): Promise<void> {
  ...
  try {
    ...
    if (msg) await msg.edit({ embeds: [endedEmbed], components: [] }).catch(() => undefined);
    if (winnerIds.length > 0) {
      ...
      await channel.send({ content: `❌ The giveaway **${giveaway.prize}** ended with no eligible entries.` });
    }
