...
export async function handleDashboardApi(req: IncomingMessage, res: ServerResponse, client: Client): Promise<boolean> {
  ...
  if (req.method === 'OPTIONS') {
    res.writeHead(204, {
      ...
      'access-control-allow-headers': 'content-type,x-dashboard-secret,x-dashboard-user',
    });
    res.end();
    return true;
  }
  ...
  const suppliedSecret = String(req.headers['x-dashboard-secret'] ?? '').trim();
  ...
  // The Website dashboard has already authenticated this Discord user and only
  // exposes guilds where Discord OAuth reported Manage Server/Administrator.
  // We only need to confirm the user is actually a member of the guild here.
  ...
  try {
    ...
    if (prefix !== undefined) {
      data.config.prefix = typeof prefix === 'string' && prefix.trim()
        ? prefix.trim().slice(0, 5)
        : undefined;
    }

    if (logChannel !== undefined) {
      data.config.logChannel = typeof logChannel === 'string' && logChannel.trim()
        ? logChannel.trim()
        : undefined;
    }
    ...
    if (autoResponders !== undefined) {
      ...
      data.autoResponders = autoResponders
        .filter((x): x is { trigger: string; response: string } =>
          !!x && typeof x === 'object' &&
    ...
    if (Object.keys(configPatch).length) {
      Object.assign(data.config as unknown as Record<string, unknown>, configPatch);
    }

    saveGuild(guildId, data);

    // Immediately read the file back so a successful response proves the
    // configuration was actually persisted for this guild.
